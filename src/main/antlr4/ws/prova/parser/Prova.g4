...
terms
    :	NEWLINE* term (NEWLINE* ',' NEWLINE* term)* //-> ^(TERM term+);
    ;
...
expr
    : aterm ((PLUS | MINUS) aterm)*
    ;

aterm
    : (MINUS? variable
        | number
        | MINUS? predicate_java_call
        | OPEN expr CLOSE
       ) (( MULT | DIV | REM ) aterm)?
   ;
...
float_literal
    : INT_LITERAL DOT INT_LITERAL EXPONENT?
    ;
